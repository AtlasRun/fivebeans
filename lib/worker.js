...
FiveBeansWorker.prototype.start = function(tubes) {
	...
	this.client.on('connect', function() {
		self.emitInfo(
			'connected to beanstalkd at ' + self.host + ':' + self.port
		);
	...
	this.client.on('error', function(err) {
		...
		self.emitWarning({
			message: 'beanstalkd connection error',
			error: err
		});
		self.emit('error', err);
	});
...
FiveBeansWorker.prototype.doNext = function() {
	...
	self.client.reserve_with_timeout(self.timeout, function(
		err,
		jobID,
		payload
	) {
		if (err) {
			...
			try {
				self.emit('job.reserved', jobID, payload.toString());
			} catch (e) {
				self.emit(
					'job.reserved',
					jobID,
			...
			try {
				...
				self.emitWarning({
					message: 'parsing job JSON',
					id: jobID,
					error: e
				});
			}
			if (!job || !_.isObject(job)) self.buryAndMoveOn(jobID);
			else if (job instanceof Array) self.runJob(jobID, job[1]);
			else self.runJob(jobID, job);
		}
	});
};
...
// issue #25
FiveBeansWorker.prototype.callHandler = function callHandler(
	handler,
	jobID,
	jobdata
) {
	if (handler.work.length === 3) {
		...
		FiveBeansWorker.prototype.doWork.call(
			this,
			patchedHandler,
			jobID,
			jobdata
...
FiveBeansWorker.prototype.doWork = function doWork(handler, jobID, jobData) {
	...
	handler
		.work(jobData, function(action, delay) {
		...
		.catch(async function(error) {
			const maxRetries = _.get(jobData, 'options.max_retries', 0);
			const retryDelay = _.get(jobData, 'options.retry_delay', 10);

			const stats = await getStats(self.client, jobID);
			const numReleases = stats.releases;

			if (numReleases < maxRetries) {
				self.releaseAndMoveOn(jobID, retryDelay);
			} else {
				error.job_id = jobID;
				error.handler_name = handlerName;
				error.payload = jobData;
...
function getStats(client, jobID) {
