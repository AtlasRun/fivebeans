...
FiveBeansWorker.prototype.doWork = function doWork(handler, jobID, jobdata)
{
	...
	handler.work(jobdata, function(action, delay)
	{
		...
		self.emitHandlerError({ message: 'exception in job handler', id: jobID, handler: handlerName, payload:jobdata, error });
...
FiveBeansWorker.prototype.emitHandlerError = function(data)
{
	data.clientid = this.id;
	this.emit('error.handler', data);
};
